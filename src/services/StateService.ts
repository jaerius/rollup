...
class StateService {
  ...
  public async updateAccountState(
    tx: SignedTransaction & { nonce: bigint }
  ): Promise<void> {
    ...
    if (fromAccount.balance < tx.amount) {
      throw new Error(
        `Insufficient balance. Account: ${tx.from}, Balance: ${fromAccount.balance}, Amount: ${tx.amount}`
      );
    }
  ...
  async saveSnapshot(transactionIndex: string): Promise<void> {
    const snapshot = {
      accounts: Array.from(this.accounts.entries()).map(
        ([address, account]) => ({
          address,
  ...
  public computeStateRoot(): string {
    const leaves = Array.from(this.accounts.entries()).map(
      ([address, account]) => {
        const balanceBytes = ethers.utils.arrayify(
          ethers.BigNumber.from(account.balance.toString()).toHexString()
        );
        const nonceBytes = ethers.utils.arrayify(
          ethers.BigNumber.from(account.nonce.toString()).toHexString()
        );
  ...
  public async revertToState(batchIndex: number): Promise<void> {
    ...
    this.accounts = new Map(
      snapshot.accounts.map((account: any) => [
        account.address,
        { balance: BigInt(account.balance), nonce: BigInt(account.nonce) },
      ])
    );
    console.log(
      Array.from(this.accounts.entries()).map(([address, account]) => ({
        address,
        balance: account.balance.toString(),
        nonce: account.nonce.toString(),
      }))
