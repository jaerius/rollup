...
class OptimisticRollup {
    ...
    for (let i = batchIndex; i < currentBatchCount; i++) {
      ...
      if (!batchData.valid) {
        ...
        const decodedTransactions =
          await this.transactionService.decodeBatchData(batchData.batchData);
        for (const tx of decodedTransactions) {
          // 트랜잭션에 tx가 invalid하지 않은 것 배열에 추가
  ...
  private async reapplyTransaction(tx: SignedTransaction) {
    ...
    const recipient = this.accounts.get(tx.to) || {
      balance: BigInt(0),
      nonce: BigInt(0),
    };
